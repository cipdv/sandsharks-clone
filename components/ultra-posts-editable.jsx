...
export default function UltraPostsEditable({ existingPlayDays = [], sponsors = [], members = [] }) {
  ...
  // Clear volunteer handlers
  const handleClearMainVolunteer = () => {
    setMainVolunteerId("")
  }

  const handleClearHelperVolunteer = () => {
    setHelperVolunteerId("")
  }
  ...
  const handleSubmit = async (e) => {
    ...
    if (hasClinic && (!clinicStartTime || !clinicEndTime)) {
      setFormError("Please fill in all clinic fields or disable the clinic option")
      return
    }
  ...
  const handleDelete = async (playDayId) => {
    if (!confirm("Are you sure you want to delete this play day? This action cannot be undone.")) {
      return
    }
  ...
  const renderCancellationForm = () => {
    ...
    return (
      <form onSubmit={handleCancelPlayDaySubmit} className="bg-white p-6 rounded-lg shadow-md">
        <h3 className="text-xl font-semibold mb-4 text-red-600">Cancel Play Day: {playDay.title}</h3>

        {formError && <div className="bg-red-50 text-red-600 p-3 rounded mb-4">{formError}</div>}

        {formSuccess && <div className="bg-green-50 text-green-600 p-3 rounded mb-4">{formSuccess}</div>}

        <div className="mb-6">
          <p className="text-gray-700 mb-4">
            Cancelling a play day will notify all members who have RSVP'd and mark the play day as cancelled in the
            system. This is different from deleting, as the play day will still be visible but marked as cancelled.
          </p>
          ...
          <div className="bg-red-50 p-4 rounded-md mb-4">
            <div className="mb-4">
              ...
              <p className="text-xs text-gray-500 mt-1">
                This reason will be included in the notification email sent to members.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-end space-x-3">
          ...
          <ActionButton className="bg-red-600 hover:bg-red-700 text-white">Confirm Cancellation</ActionButton>
        </div>
      </form>
    )
  }
  ...
  return (
    <div className="space-y-8">
      ...
      {formSuccess && !isCreating && !editingPlayDayId && !isCancelling && (
        <div className="bg-green-50 text-green-600 p-3 rounded">{formSuccess}</div>
      )}
      ...
      {(isCreating || editingPlayDayId) && (
        <form onSubmit={handleSubmit} className="bg-white p-4 sm:p-6 rounded-lg shadow-md">
          <h3 className="text-xl font-semibold mb-4">{isCreating ? "Create New Play Day" : "Edit Play Day"}</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Main Volunteer</label>
              <div className="flex gap-2">
                ...
                {mainVolunteerId && (
                  <button
                    type="button"
                    onClick={handleClearMainVolunteer}
                    className="px-3 py-2 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300 flex-shrink-0"
                    title="Clear volunteer"
                  >
                    Clear
                  </button>
                )}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Helper Volunteer</label>
              <div className="flex gap-2">
                <select
                ...
                {helperVolunteerId && (
                  <button
                    type="button"
                    onClick={handleClearHelperVolunteer}
                    className="px-3 py-2 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300 flex-shrink-0"
                    title="Clear volunteer"
                  >
                    Clear
          ...
          <div className="mb-6">
            ...
            {hasClinic && (
              <div className="bg-yellow-50 p-4 rounded-md">
                <h4 className="font-medium text-yellow-800 mb-3">Beginner Clinic Details</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Clinic Start Time *</label>
                    <input
                      type="time"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Clinic End Time *</label>
                    <input
                      type="time"
                      value={clinicEndTime}
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Clinic Courts *</label>
                    <input
                      type="text"
                      value={clinicCourts}
                ...
                {sortedPlayDays.length === 0 ? (
                  ...
                  sortedPlayDays.map((playDay) => (
                    <tr key={playDay.id} className={playDay.is_cancelled ? "bg-red-50" : ""}>
                        ...
                        <div className="text-sm text-gray-500">
                          {playDay.date
                            ? (() => {
                                ...
                                const dateParts = playDay.date.split("T")[0].split("-")
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-500">
                          {formatTime(playDay.startTime)} - {formatTime(playDay.endTime)}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        {playDay.hasClinic ? (
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        {playDay.sponsorName && <div className="text-sm text-gray-500">{playDay.sponsorName}</div>}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-xs text-gray-500">
                          {playDay.mainVolunteer && (
                            <div>
                              Main: {playDay.mainVolunteer.first_name} {playDay.mainVolunteer.last_name}
                            </div>
                          )}
                          {playDay.helperVolunteer && (
                            <div>
                              Helper: {playDay.helperVolunteer.first_name} {playDay.helperVolunteer.last_name}
                            </div>
                          )}
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex justify-end space-x-2">
                          ...
                          <button onClick={() => handleDelete(playDay.id)} className="text-red-600 hover:text-red-900">
                            Delete
                          </button>
                        </div>
                      </td>
                ...
                sortedPlayDays.map((playDay) => (
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      <div>
                        <span className="font-medium">Time:</span> {formatTime(playDay.startTime)} -{" "}
                        {formatTime(playDay.endTime)}
                      </div>
                      <div>
                        <span className="font-medium">Courts:</span> {playDay.courts}
                      </div>
                      <div>
                        <span className="font-medium">Clinic:</span> {playDay.hasClinic ? "Yes" : "No"}
                      </div>
                      <div>
                        <span className="font-medium">RSVPs:</span> {playDay.replies?.length || 0}
                      </div>
                      {playDay.sponsorName && (
                        <div className="col-span-2">
                          <span className="font-medium">Sponsor:</span> {playDay.sponsorName}
                        </div>
                      )}
