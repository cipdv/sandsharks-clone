...
export async function sendWeeklyPlayDayEmails() {
  try {
    // Prevent duplicate sends within the debounce interval
    const now = Date.now()
    if (now - lastEmailSentTimestamp < DEBOUNCE_INTERVAL) {
      console.log(
        `Preventing duplicate email send. Last email was sent ${(now - lastEmailSentTimestamp) / 1000} seconds ago.`,
      )
      return {
        success: false,
        error: "Duplicate email prevention - email was sent too recently",
        isDuplicate: true,
      }
    }
    ...
    if (membersResult.rows.length === 0) {
      console.log("No members found who want to receive emails. No emails sent.")
      return {
        success: true,
        totalEmails: 0,
